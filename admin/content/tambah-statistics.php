...
<html>
<head>
    <title><?= $idEdit > 0 ? 'Edit' : 'Tambah' ?> Statistic</title>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css">
</head>
<body>
<div class="container mt-4">
    <h2><?= $idEdit > 0 ? 'Edit' : 'Tambah' ?> Statistic</h2>
    <?= $msg ?>

    <form method="post">
        <div class="mb-3">
            <label class="form-label">Icon (Bootstrap Icon class)</label>
            <input type="text" name="icon" class="form-control" value="<?= htmlspecialchars($editData['icon'] ?? '') ?>" placeholder="contoh: bi bi-star">
        </div>
        <div class="mb-3">
            <label class="form-label">Value</label>
            <input type="number" name="value" class="form-control" value="<?= htmlspecialchars($editData['value'] ?? 0) ?>" min="0">
        </div>
        <div class="mb-3">
            <label class="form-label">Label</label>
            <input type="text" name="label" class="form-control" value="<?= htmlspecialchars($editData['label'] ?? '') ?>" required>
        </div>
