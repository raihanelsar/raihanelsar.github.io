...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (in_array($type, $ext_allowed)) {
            ...
            $msg = 'Format gambar tidak didukung. Gunakan PNG, JPG, atau JPEG.';
        }
    }
    ...
    if (!empty($_FILES['cv']['name'])) {
        $ext_allowed = ["application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"];
        $type = mime_content_type($_FILES['cv']['tmp_name']);

        if (in_array($type, $ext_allowed)) {
            $cvFile = save_upload('cv', 'uploads');

            if (!empty($curr['cv']) && file_exists('uploads/' . $curr['cv'])) {
                @unlink('uploads/' . $curr['cv']);
            }
        } else {
            $msg = 'Format CV tidak didukung. Gunakan PDF atau DOC/DOCX.';
        }
    }

    // Tentukan file yang dipakai
    $imageToUse = $img ?: ($curr['image'] ?? '');
    $cvToUse    = $cvFile ?: ($curr['cv'] ?? '');
    ...
    if (empty($msg)) {
        if (!empty($curr)) {
            ...
            $sql = "INSERT INTO about (title, description, image, birthday, website, phone, city, age, degree, email, cv) 
                    VALUES ('$title', '$description', '$imageToUse', '$birthday', '$website', '$phone', '$city', $age, '$degree', '$email', '$cvToUse')";
            mysqli_query($koneksi, $sql);
        }
...
<div class="container">
    ...
    <form method="post" enctype="multipart/form-data" class="row g-3">
        ...
        <div class="col-md-6">
            ...
            <?php if (!empty($curr['cv'])): ?>
                <div class="mt-2">
                    <a href="uploads/<?= htmlspecialchars($curr['cv']) ?>" target="_blank" class="btn btn-sm btn-outline-primary">Lihat CV</a>
                </div>
            <?php endif; ?>
        </div>
        <div class="col-md-6">
